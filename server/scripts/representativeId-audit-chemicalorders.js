#!/usr/bin/env node
// Read-only audit: counts ChemicalOrder documents by the runtime BSON type
// of representativeId (objectId vs string vs nullish vs other) and samples
// up to 10 string-typed rows so a backfill plan can be sized. No writes.
//
// Usage:
//   node -r dotenv/config server/scripts/representativeId-audit-chemicalorders.js
//
// Exit codes:
...
(async () => {
    ...
    const col = mongoose.connection.db.collection('chemicalorders');
